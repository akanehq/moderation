...
class CustomVoice {
  ...
  @Guard(
    ...
    hasPermission({
      for: GuardFor.Both,
      clientPermissions: ['ManageChannels', 'MoveMembers'],
  ...
  async setup(
    ...
    if (parentExists) {
      await interaction.reply(
        LL.ERRORS.CUSTOMVOICE_PARENT_ALREADY_CONFIGURED()
      );
    ...
    await interaction.reply({
      content: LL.CUSTOMVOICE_SUCCESS_SETUP(),
      ephemeral: true,
    });
  }
  ...
  async updateTemplate(
    ...
    if (!customVoiceExists) {
      ...
      await interaction.reply({
        content: LL.ERRORS.CUSTOMVOICE_PARENT_NOT_FOUND(),
        ephemeral: true,
      });
    ...
    await interaction.reply({
      content: LL.CUSTOMVOICE_SUCCESS_UPDATE_TEMPLATE(),
      ephemeral: true,
    });
  }
